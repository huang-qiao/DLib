...
public:
    ...
    :m_bits(alloc),
     m_num_bits(0)
    {
        using DBoost::detail::dynamic_bitset_impl::value_to_type;
        using DBoost::detail::dynamic_bitset_impl::is_numeric;
...
*/
    ...
    void m_append(BlockInputIterator first, BlockInputIterator last, std::forward_iterator_tag)
    {
        ...
        std::size_t d = DBoost::detail::distance(first, last);
...
#define BOOST_NOEXCEPT // do nothing
...
private:
        ...
        size_type num_bits)
    {
        ...
        for( ; i < m; ++i) {
            ...
            const char c = s[(pos + m - 1) - i];

            //assert( Tr::eq(c, one) || Tr::eq(c, BOOST_DYNAMIC_BITSET_WIDEN_CHAR(fac, '0')) );
            assert(c == one);
                                 ...
                                 const Allocator& alloc*/)
    {
        ...
        typedef DBoost::detail::dynamic_bitset_impl
...
template <typename Block, typename Allocator>
inline dynamic_bitset<Block, Allocator>::
//dynamic_bitset(dynamic_bitset<Block, Allocator>&& b) : m_bits(DBoost::move(b.m_bits)), m_num_bits(DBoost::move(b.m_num_bits))
dynamic_bitset(dynamic_bitset<Block, Allocator>&& b) : m_bits(std::move(b.m_bits)), m_num_bits(std::move(b.m_num_bits))
{
...
{
    //if (DBoost::addressof(b) == this) { return *this; }
    if (std::addressof(b) == this) { return *this; }

    //m_bits = DBoost::move(b.m_bits);
    //m_num_bits = DBoost::move(b.m_num_bits);
...
#define BOOST_THROW_EXCEPTION std::exception
...
to_ulong() const
{
  ...
  if (find_next(ulong_width - 1) != npos)
    BOOST_THROW_EXCEPTION(std::overflow_error("DBoost::dynamic_bitset::to_ulong overflow"));
...
struct max_pow2_less {
    ...
    //BOOST_STATIC_CONSTANT(int, value = c ? (max_pow2_less< c*p, 2*c*n>::value) : n);
...
int integer_log2(T x) {
    assert(x > 0);
    const int val = std::numeric_limits<T>::digits;
    const int n = max_pow2_less<val, 4>::value;
...
dynamic_bitset<Block, Allocator>::m_do_find_from(size_type first_block) const
{
    ...
    return i * bits_per_block + static_cast<size_type>(DBoost::lowest_bit(m_bits[i]));
